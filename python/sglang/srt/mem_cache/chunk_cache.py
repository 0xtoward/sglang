...
class ChunkCache(BasePrefixCache):
    ...
    def cache_finished_req(self, req: Req, insert: bool = True):
        if req.actual_kv_len is not None:
            # KVPress: compacted gap-free physical cache (kept prefill + decode tokens).
            kv_len = req.actual_kv_len
        else:
            # The last output token's KV is not stored yet.
            kv_len = len(req.origin_input_ids) + max(len(req.output_ids) - 1, 0)
        kv_indices = self.req_to_token_pool.req_to_token[req.req_pool_idx, :kv_len]
